...
<div class="container">
	<div class="content-container">
	    <h1 class="stock-header">삭제 요청 리스트</h1>
		
		<form name="f1" action="delete_admin.do" class="flex-container" method="post">
	    	<div class="flex-row">
	...
	<div class="flex-subrow custom-button-row">
		...
		<div align="left" class="results-heading">
		    <font size="2">검색 결과</font>&nbsp;&nbsp;총 ${deleteRequestCount} 건
		</div>
		<div class="scroll flex-container content-table">
		    ...
		    <c:if test="${empty deleteRequestProduct}">
		        <div class="flex-row">
		            <div class="flex-cell" colspan="10">등록된 상품이 없습니다.</div>
		        </div>
		    </c:if>
		    <c:forEach var="dto" items="${deleteRequestProduct}">
		        <div class="flex-row">
            ...
            <img src="data:image/jpeg;base64,${dto.encodedImage}" width="40" height="40">
		            ...
		            <div class="flex-cell">
	                    <c:if test="${dto.product_approval_status=='dr'}">
				            <input type="button" class="update-button" style="background-color: #50E5B4; color: white;" value="확인" onclick="window.location='admin_request_cancle.do?product_num=${dto.product_num }'">
                    	</c:if>
		            </div>
		        </div>
		    </c:forEach>
